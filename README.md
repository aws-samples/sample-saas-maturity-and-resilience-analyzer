...
In addition to Well-Architected lens reviews, the analyzer supports **domain-specific resilience reviews** (Data, EKS, Network, SaaS, Serverless) and a **SaaS Maturity Assessment** that evaluates multi-tenant SaaS architectures across 5 pillars with 1-5 maturity ratings, generating PDF reports with executive summaries, pillar analysis, and prioritized improvement roadmaps.
...
---
- **NEW** — SaaS Maturity Assessment:
  - Assess SaaS maturity across 5 pillars (16 questions)
  - Pillars: Tenant Lifecycle Management, Tenant Isolation Management, Billing & Metering, Operations/Metrics/Analytics, DevOps Governance
  - 5-level maturity rating (1=Manual/Ad-hoc → 5=Optimized/Continuous) with confidence scoring
  - Radar chart visualization of per-pillar maturity scores
  - Process assessment notes flag areas requiring organizational review beyond IaC
  - PDF report generation with executive summary, pillar analysis, and prioritized improvement roadmap
  - Assessment-specific chat assistant understands maturity levels and process assessment context
---
- **NEW** — Domain-Specific Resilience Reviews:
  - Specialized best-practice assessments for targeted infrastructure domains
  - Available domains: Data Resilience, EKS Resilience, Network Resilience, SaaS Resilience, Serverless Resilience
  - Questions loaded from S3 (with fallback to built-in configuration)
  - Same analysis engine as WA lens reviews — KB retrieval, Bedrock analysis, recommendations
---
- **NEW** — Batch Parallel Analysis Processing:
  - Questions analyzed in parallel batches for faster results
  - Configurable batch sizes: `BATCH_SIZE` (default 5) for lens/domain reviews, `SMA_BATCH_SIZE` (default 3) for SaaS Maturity Assessment
  - Exponential backoff retry for Bedrock throttling (30s base, up to 5 retries)
  - ~2.5x speedup for SaaS Maturity Assessment (16 questions in ~7 min vs ~17 min sequential)
---
...
The analyzer supports three categories of assessments, selectable from a grouped dropdown:

### Well-Architected Lens Reviews
Analyze IaC templates against AWS Well-Architected best practices using official AWS lenses (SaaS, Serverless, IoT, ML, etc.). Each question evaluates alignment with specific best practices and provides recommendations.

### Domain-Specific Resilience Reviews
Targeted assessments for specific infrastructure domains:
- **Data Resilience** — 15 best-practice questions for data protection and recovery
- **EKS Resilience** — 15 best-practice questions for Kubernetes workload resilience
- **Network Resilience** — 15 best-practice questions for network architecture resilience
- **SaaS Resilience** — Best-practice questions for SaaS-specific resilience patterns
- **Serverless Resilience** — Best-practice questions for serverless architecture resilience

### SaaS Maturity Assessment
A maturity-model assessment with 16 questions across 5 pillars:

| Pillar | Questions | Focus |
|--------|-----------|-------|
| Tenant Lifecycle Management | 3 | Onboarding, offboarding, tenant provisioning automation |
| Tenant Isolation Management | 3 | Data isolation, compute isolation, cross-tenant access controls |
| Billing and Metering | 4 | Usage tracking, billing automation, metering granularity |
| Operations, Metrics & Analytics | 3 | Tenant-aware monitoring, operational dashboards, analytics |
| DevOps Governance | 3 | CI/CD pipelines, deployment strategies, team collaboration |

Each question receives a maturity level (1-5), confidence rating (high/medium/low), and process assessment notes for areas requiring organizational review beyond IaC evidence.

### Question Loading
Assessment questions are loaded from S3 (Analysis Storage bucket) with automatic fallback to built-in configuration in `domain.config.ts`. Question files are stored in the `ecs_fargate_app/assessment_questions/` folder and deployed to S3 via CDK `BucketDeployment`.

## Installation and Deployment

You have three options for deploying this solution:
- Option 1: Using a CloudFormation Deployment Stack
- Option 2: Using a Deployment Script (Recommended)
- Option 3: Manual Deployment

### Option 1: Using a CloudFormation Deployment Stack

> **Note**: This deployment option uses a CloudFormation template that launches an EC2 instance to clone this repository and deploy the full CDK stack. It is the simplest approach but takes slightly longer (15-20 minutes).

<details>

<summary>Expand for Option 1 instructions:</summary>

#### Prerequisites

You must enable **AWS Bedrock Model Access** to the following LLM models in your AWS region:
* **Titan Text Embeddings V2**
* **Claude Sonnet 4.5** (default) or Claude 3.7 Sonnet, Claude 3.5 Sonnet v2
...
1. Download the CloudFormation template: [iac-analyzer-deployment-stack.yaml](https://github.com/aws-samples/sample-saas-maturity-and-resilience-analyzer/blob/main/cfn_deployment_stacks/iac-analyzer-deployment-stack.yaml)
...
4. On the "Specify stack details" page. Enter or change the stack name, then:
   - Change the stack parameters as needed. Check the [CloudFormation Configuration Parameters](#cloudformation-configuration-parameters) section below for details
     ...
     - **Model Selection Note:** The tool currently defaults to Claude Sonnet 4.5 (`global.anthropic.claude-sonnet-4-5-20250929-v1:0`). If you want to use a different model, update the model ID in the stack "Amazon Bedrock Model ID" configuration parameter. Verify model availability in your region before deployment.
...
> **⚠️ Important — Configure `config.ini` Before Deploying:** The `config.ini` file ships with `authentication = True` by default, which requires an ACM certificate and Cognito configuration. If you do not need authentication, set `authentication = False` in `config.ini` before deploying. Otherwise, configure the required authentication fields first — see the [Configuration Options For Manual Deployments section](#configuration-options-for-manual-deployments) for details.

1. Clone the Repository
```bash
git clone https://github.com/aws-samples/sample-saas-maturity-and-resilience-analyzer.git
cd sample-saas-maturity-and-resilience-analyzer
```

2. Configure `config.ini` (required):
```ini
# For a quick deployment without authentication, set:
authentication = False
auth_type = none

# For production deployments with authentication, see the
# "Configuration Options For Manual Deployments" section below.
```
...
> **Note**: This section applies to Option 1 (CloudFormation Deployment Stack), which is currently unavailable for this version. It is retained here for reference when the solution is published to `aws-samples`.
...
- **Amazon Bedrock Model ID** (`ModelId`)
  ...
  - **Note**: This application has been primarily tested with Claude Sonnet 4.5, Claude 3.7 Sonnet, and Claude 3.5 Sonnet v2. While other Bedrock models may work, using different models might lead to unexpected results.
...
> **Note:** This application has been primarily tested with Claude Sonnet 4.5, Claude 3.7 Sonnet, and Claude 3.5 Sonnet v2. While other Bedrock models may work, using different models might lead to unexpected results. The default model ID is set to `global.anthropic.claude-sonnet-4-5-20250929-v1:0`.
...
> **Architecture Note**: The local development container images default to `arm64` (Apple Silicon). If you are on an Intel/AMD machine, edit the `PLATFORM` argument in `ecs_fargate_app/finch/backend.dev.Dockerfile` and `ecs_fargate_app/finch/frontend.dev.Dockerfile` from `arm64` to `amd64` before building.
...
> **SaaS Maturity Assessment & Domain Reviews Note**: If you deployed the local development KB storage stack (from `local_development/`), the assessment question files are automatically uploaded to S3 via CDK — no manual steps needed. SaaS KB documents are fetched by the KB synchronizer Lambda at sync time.
>
> If you are using an existing stack that was deployed before these features were added, you need to manually upload the assessment question files to your Analysis Storage bucket:
> ```bash
> aws s3 sync ecs_fargate_app/assessment_questions/ s3://<your-analysis-storage-bucket>/
> ```
> If the S3 question files are not present, the application will gracefully fall back to the built-in questions in `domain.config.ts`.

> **Batch Size Configuration**: You can optionally add these to your `.env` file to tune parallel analysis performance:
> ```ini
> BATCH_SIZE=5              # Batch size for WA lens and domain reviews (default: 5, range: 1-12)
> SMA_BATCH_SIZE=3          # Batch size for SaaS Maturity Assessment (default: 3, range: 1-12)
> ```
...
|--------|---------|-------------|
| `ecs_fargate_app/assessment_questions/` | Assessment question JSON files (SaaS Maturity Assessment, domain reviews) | Analysis Storage S3 bucket (root) |
| `saas_kb_docs/` | Source URL manifest for SaaS KB documents (fetched by Lambda at deploy time) | WA Docs S3 bucket (`softwareasaservice/additional_saas_docs/`) |

> **Note**: Well-Architected Framework reference documents and SaaS KB documents are fetched at deploy/sync time by the KB synchronizer Lambda from canonical AWS URLs. See `saas_kb_docs/README.md` for the full list of source documents.
