...
export class StorageService {
  ...
  constructor(
    ...
    this.config = {
      enabled: this.configService.get<boolean>('storage.enabled', true),
    ...
    this.logger.log(`Storage configuration: enabled=${this.config.enabled}, bucket=${this.config.bucket}, table=${this.config.table}`);
  }
  ...
  async storeAnalysisResults(
    ...
    if (!this.config.enabled) {
      this.logger.warn('Storage is not enabled - cannot store analysis results');
      throw new Error('Storage is not enabled');
    }

    const s3Client = this.awsConfig.createS3Client();
    const s3Locations = this.getS3Locations(userId, fileId);
    const s3Key = s3Locations.getAnalysisResultsPath(lensAlias);

    this.logger.log(`Storing analysis results: bucket=${this.config.bucket}, key=${s3Key}, resultsCount=${Array.isArray(results) ? results.length : 'N/A'}`);
    ...
    try {
      ...
      await upload.done();
      this.logger.log(`Successfully stored analysis results to S3: ${s3Key}`);
    } catch (error) {
      this.logger.error(`Error storing analysis results to S3 (bucket: ${this.config.bucket}, key: ${s3Key}):`, error);
  ...
  async getAnalysisResults(userId: string, fileId: string, lensAlias: string): Promise<any> {
    if (!this.config.enabled) {
      this.logger.warn('Storage is not enabled - cannot get analysis results');
      throw new Error('Storage is not enabled');
    }
    ...
    this.logger.log(`Getting analysis results: bucket=${this.config.bucket}, key=${s3Key}`);
    ...
    try {
      ...
      this.logger.log(`Successfully retrieved analysis results from S3: ${s3Key}, resultsCount=${Array.isArray(parsedResults) ? parsedResults.length : 'N/A'}`);
      return parsedResults;
    } catch (error) {
      if (error.name === 'NoSuchKey') {
        this.logger.warn(`Analysis results file not found: bucket=${this.config.bucket}, key=${s3Key}`);
        throw new Error(`Analysis results not found for lens ${lensAlias}`);
      }
      this.logger.error(`Error getting analysis results from S3 (bucket: ${this.config.bucket}, key: ${s3Key}):`, error);
