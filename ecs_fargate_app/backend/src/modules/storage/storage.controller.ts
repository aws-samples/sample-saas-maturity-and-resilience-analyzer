...
export class StorageController {
    ...
    async getWorkItem(
        ...
        try {
            ...
            console.log(`Checking lens condition: lensAlias=${lensAlias}, usedLenses=${JSON.stringify(workItem.usedLenses)}`);
            if (lensAlias && workItem.usedLenses?.some(lens => lens.lensAlias === lensAlias)) {
                console.log(`Lens found in usedLenses. Checking analysis status: ${JSON.stringify(workItem.analysisStatus)}`);
                ...
                if (workItem.analysisStatus?.[lensAlias] === 'COMPLETED' || workItem.analysisStatus?.[lensAlias] === 'PARTIAL') {
                    console.log(`Analysis status is COMPLETED/PARTIAL for ${lensAlias}. Calling getAnalysisResults...`);
                    try {
                    ...
                    console.log(`Analysis status is not COMPLETED/PARTIAL for ${lensAlias}: ${workItem.analysisStatus?.[lensAlias]}`);
                }
            } else {
                console.log(`Lens condition failed: lensAlias=${lensAlias}, lensExists=${workItem.usedLenses?.some(lens => lens.lensAlias === lensAlias)}`);
            ...
            // Otherwise, get data for any available lens
                ...
                console.log(`Using default lens logic. Available lenses: ${JSON.stringify(workItem.usedLenses)}`);

                // Get the first available lens
                const defaultLens = workItem.usedLenses[0].lensAlias;
                console.log(`Default lens: ${defaultLens}, analysis status: ${workItem.analysisStatus?.[defaultLens]}`);

                if (workItem.analysisStatus?.[defaultLens] === 'COMPLETED' || workItem.analysisStatus?.[defaultLens] === 'PARTIAL') {
                    console.log(`Default lens analysis is COMPLETED/PARTIAL. Calling getAnalysisResults...`);
                    try {
                        response.analysisResults = await this.storageService.getAnalysisResults(
                            userId,
                            fileId,
