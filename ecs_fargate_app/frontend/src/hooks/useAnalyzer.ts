...
export const useAnalyzer = () => {
  ...
  const analyze = async (
    ...
    try {
      ...
      if (!workloadId) {
        ...
        // This is a user-provided workload, mark as protected
  ...
  const downloadRecommendations = async (originalFileName: string = 'unknown_file', lensAlias?: string) => {
    ...
    try {
      const safeFileName = originalFileName.replace(/\./g, '_');
      const isSMA = analysisResults.length > 0 && analysisResults[0].maturityLevel !== undefined;

      const csv = isSMA
        ? await analyzerApi.generateSMARecommendations(analysisResults)
        : await analyzerApi.generateRecommendations(analysisResults);

      const blob = new Blob([csv], { type: 'text/csv' });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      const prefix = isSMA ? 'SMA_Assessment' : lensAlias || 'Analysis';
      a.download = `IaCAnalyzer_${prefix}_Results_${safeFileName}.csv`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const generateSMAReport = async (originalFileName: string = 'unknown_file', outputLanguage: string = 'en') => {
    if (!analysisResults) return;

    setIsGeneratingReport(true);
    try {
      const base64String = await analyzerApi.generateSMAReport(analysisResults, originalFileName, outputLanguage);
      if (!base64String) {
        throw new Error('No report data received');
      }
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `SMA_Report_${safeFileName}.pdf`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to generate SMA report');
    } finally {
      setIsGeneratingReport(false);
    }
  };
