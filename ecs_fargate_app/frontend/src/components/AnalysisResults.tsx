...
export const AnalysisResults: React.FC<AnalysisResultsProps> = ({ results, isAnalyzing, onDownloadRecommendations, onGenerateIacDocument, onGenerateSMAReport, isDownloading, isImplementing, isGeneratingReport, isLoadingDetails, setIsLoadingDetails, uploadedFileType, selectedIaCType, setError, fileId, fileName, domain, domainName, outputLanguage }) => {
  ...
  // Detect SMA results — if the first result has maturityLevel, render SMA view
  const isSMA = results.length > 0 && results[0].maturityLevel !== undefined;
  if (isSMA) {
    return (
      <SMAResults
        results={results}
        isAnalyzing={isAnalyzing}
        onDownloadRecommendations={onDownloadRecommendations}
        onGenerateSMAReport={onGenerateSMAReport}
            ...
            cell: item => (
              <Link external href={
                domain === 'wellarchitected' 
                  ...
                  : `https://docs.aws.amazon.com/search/doc-search.html?searchPath=documentation-guide&searchQuery=${encodeURIComponent(`"${domainName}"+"${item.pillar}"`)}`
              }>
                {item.name}
              </Link>
            ),
