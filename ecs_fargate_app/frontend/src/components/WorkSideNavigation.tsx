...
export interface WorkSideNavigationProps {
  activeFileId?: string;
  onItemSelect: (workItem: WorkItem, loadResults?: boolean, domain?: string, domainName?: string) => void;
...
export const WorkSideNavigation = forwardRef<WorkSideNavigationRef, WorkSideNavigationProps>((
  ...
  const navigationItems: SideNavigationProps.Item[] = [
    ...items.map(item => {
      ...
      const fileItems = [
        // Domains section with badges for each domain
        {
          type: 'link' as const,
          text: 'Domains',
          href: `/domains/${item.fileId}`,
          key: `domains-${item.fileId}`,
          info: (
            <SpaceBetween direction="horizontal" size="xxxs">
              {item.analysisStatus && Object.keys(item.analysisStatus).map(domainId => {
                const domainNames: Record<string, string> = {
                  'saas_resilience': 'SaaS',
                  'eks_resilience': 'EKS',
                  'serverless_resilience': 'Serverless',
                  'data_resilience': 'Data',
                  'network_resilience': 'Network'
                };
                return (
                  <Badge
                    key={domainId}
                    color={getBadgeColor(item.analysisStatus?.[domainId])}
                  >
          ...
          info: (
            <Button
              ...
              onClick={async (e: any) => {
                ...
                try {
                  // Find a domain with completed analysis
                  let domainToLoad = 'saas_resilience'; // Default domain
                  let domainNameToLoad = 'SaaS Resilience';

                  if (item.analysisStatus) {
                    const completedDomain = Object.keys(item.analysisStatus).find(
                      domainKey => item.analysisStatus?.[domainKey] === 'COMPLETED' || item.analysisStatus?.[domainKey] === 'PARTIAL'
                    );
                    if (completedDomain) {
                      domainToLoad = completedDomain;
                      // Map domain ID to display name
                      const domainNames: Record<string, string> = {
                        'saas_resilience': 'SaaS Resilience',
                        'eks_resilience': 'EKS Resilience',
                        'serverless_resilience': 'Serverless Resilience',
                        'data_resilience': 'Data Resilience',
                        'network_resilience': 'Network Resilience'
                      };
                      domainNameToLoad = domainNames[completedDomain] || completedDomain;
                    }
                  }
