...
def handler(event, context):
    ...
    # -------------------------------------------------------------------
    # Fetch additional SaaS KB documents (whitepapers + blog posts)
    # These are fetched at deploy/sync time rather than bundled in the repo
    # to avoid redistributing copyrighted content.
    # -------------------------------------------------------------------

    saas_kb_prefix = "softwareasaservice/additional_saas_docs"

    # Group 1: AWS whitepapers with canonical PDF URLs
    saas_whitepaper_docs = [
        {
            "url": "https://docs.aws.amazon.com/pdfs/whitepapers/latest/saas-architecture-fundamentals/saas-architecture-fundamentals.pdf",
            "fileName": "saas-architecture-fundamentals.pdf",
            "title": "SaaS Architecture Fundamentals",
            "type": "whitepaper",
        },
        {
            "url": "https://docs.aws.amazon.com/pdfs/whitepapers/latest/multi-tenant-saas-storage-strategies/multi-tenant-saas-storage-strategies.pdf",
            "fileName": "multi-tenant-saas-storage-strategies.pdf",
            "title": "SaaS Storage Strategies",
            "type": "whitepaper",
        },
        {
            "url": "https://docs.aws.amazon.com/pdfs/whitepapers/latest/saas-journey-framework/saas-journey-framework.pdf",
            "fileName": "saas-journey-framework.pdf",
            "title": "SaaS Journey Framework",
            "type": "whitepaper",
        },
        {
            ...
            "fileName": "saas-tenant-isolation-strategies.pdf",
            "title": "SaaS Tenant Isolation Strategies",
            "type": "whitepaper",
        },
    ]

    # Group 2: AWS blog posts (fetched as HTML)
    saas_blog_docs = [
        {
            "url": "https://aws.amazon.com/blogs/architecture/tenant-portability-move-tenants-across-tiers-in-a-saas-application/",
            "fileName": "tenant-portability-move-tenants-across-tiers.html",
            "title": "Tenant portability: Move tenants across tiers in a SaaS application",
            "type": "blog",
        },
        {
            "url": "https://aws.amazon.com/blogs/apn/tenant-switching-and-custom-permissions-in-a-multi-tenant-serverless-application/",
            "fileName": "tenant-switching-custom-permissions-serverless.html",
            "title": "Tenant Switching and Custom Permissions in a Multi-Tenant Serverless Application",
            "type": "blog",
        },
        {
            "url": "https://aws.amazon.com/blogs/architecture/simplify-multi-tenant-encryption-with-a-cost-conscious-aws-kms-key-strategy/",
            "fileName": "simplify-multi-tenant-encryption-kms.html",
            "title": "Simplify multi-tenant encryption with a cost-conscious AWS KMS key strategy",
            "type": "blog",
        },
        {
            "url": "https://aws.amazon.com/blogs/architecture/6000-aws-accounts-three-people-one-platform-lessons-learned/",
            "fileName": "6000-aws-accounts-three-people-one-platform.html",
            "title": "6,000 AWS accounts, three people, one platform: Lessons learned",
            "type": "blog",
        },
        {
            "url": "https://aws.amazon.com/blogs/apn/protect-your-saas-data-and-leverage-it-to-gain-insights-using-own-and-aws/",
            "fileName": "protect-saas-data-leverage-insights.html",
            "title": "Protect Your SaaS Data and Leverage it to Gain Insights Using Own and AWS",
            "type": "blog",
        },
        {
            "url": "https://aws.amazon.com/blogs/apn/migrating-applications-to-saas-rethinking-your-design/",
            "fileName": "migrating-applications-to-saas-rethinking-design.html",
            "title": "Migrating Applications to SaaS: Rethinking Your Design",
            "type": "blog",
        },
        {
            "url": "https://aws.amazon.com/blogs/architecture/lets-architect-building-multi-tenant-saas-systems/",
            "fileName": "lets-architect-building-multi-tenant-saas-systems.html",
            "title": "Let's Architect! Building multi-tenant SaaS systems",
            "type": "blog",
        },
    ]

    def create_saas_metadata(doc):
        """Create metadata JSON for a SaaS KB document."""
        return json.dumps(
            {
                "metadataAttributes": {
                    "lens_name": "SaaS Lens",
                    "lens_arn": "arn:aws:wellarchitected::aws:lens/softwareasaservice",
                    "lens_author": "AWS",
    ...
    # Fetch and upload SaaS whitepapers (PDF)
    print("Fetching SaaS KB whitepaper documents...")
    for doc in saas_whitepaper_docs:
        try:
            content = download_file(doc["url"])
            upload_to_s3(bucket_name, doc["fileName"], content, prefix=saas_kb_prefix)
            # Upload metadata sidecar
            metadata_content = create_saas_metadata(doc)
            upload_to_s3(
                bucket_name,
                f"{doc['fileName']}.metadata.json",
                metadata_content,
                prefix=saas_kb_prefix,
            )
            print(f"  ✓ {doc['title']}")
        except Exception as e:
            print(f"  ✗ Failed to fetch {doc['title']}: {e}")

    # Fetch and upload SaaS blog posts (HTML)
    print("Fetching SaaS KB blog documents...")
    for doc in saas_blog_docs:
        try:
            content = download_file(doc["url"])
            upload_to_s3(bucket_name, doc["fileName"], content, prefix=saas_kb_prefix)
            # Upload metadata sidecar
    ...
    # After all lenses and SaaS docs are processed, start the ingestion job
